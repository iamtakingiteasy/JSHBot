package org.eientei.jshbot.bundles.utils.tree;
...
public class Tree<K,V> implements Iterable<Tree.Node<K, V>> {
    ...
    public void addAll(Tree<K,V> otherTree) {
        List<Node<K,V>> otherNodes = otherTree.flattern();
        for (Node<K,V> n : otherNodes) {
            insert(n.getPath(),n.getData());
        }
    }

    public List<Node<K,V>> flattern() {
        List<Node<K,V>> nodes = new ArrayList<Node<K,V>>();
        flattern(nodes,root);
        return nodes;
    }

    protected void flattern(List<Node<K,V>> nodes, Node<K,V> relRoot) {
        for (Node<K,V> n : relRoot) {
            if (!n.isEmpty()) {
                nodes.add(n);
            }
            flattern(nodes, n);
    ...
    public V set(List<K> keys, V value) {
    ...
    public Node<K,V> insert(List<K> keys, V value) {
    ...
    public static class Node<K,V> implements Iterable<Node<K,V>> {
        ...
        public Node(Node<K, V> parent, K key) {
            this.parent = parent;
        ...
        public Node<K,V> addChild(K key) {
            Node<K,V> n = new Node<K, V>(this, key);
            children.put(key, n);
            return n;
        }
